...
Array_list *array_list_create(size_t item_size, size_t initial_capacity)
{
    ...
    if(!list)
        ERROR_RETURN(NULL, "Couldn't allocate memory for the Array_list\n");
...
size_t array_list_append(Array_list *list, void *item)
{
    if(list->len == list->capacity)
    {
        ...
        if(!items)
            ERROR_RETURN(-1, "Couldn't allocate memory for the Array_list\n");
...
u8 array_list_remove(Array_list *list, size_t index)
{
    ...
    if(index >= list->len)
        ERROR_RETURN(1, "Index out of boundsn\n");
