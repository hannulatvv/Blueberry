...
void physics_update(void)
{
    ...
    for(u32 i =  0; i < state.body_list->len; i++){
        ...
        body->aabb.position[0] += body->acceleration[0] * global.time.delta;
        body->aabb.position[1] += body->acceleration[1] * global.time.delta;
    }
}
...
size_t physics_body_create(vec2 position, vec2 size)
{
    ...
    if(array_list_append(state.body_list, &body) == (size_t)-1)
        ERROR_EXIT("Couldn't append body to the list\n");
